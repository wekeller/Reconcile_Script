...
try:
    ...
    if len(verReconcileList) > 0:
        ...
        arcpy.ReconcileVersions_management(Sworkspace, "ALL_VERSIONS", StargetVersion, verReconcileList,
                                           "LOCK_ACQUIRED",
        ...
        arcpy.AnalyzeDatasets_management(Sworkspace, "SYSTEM", "", "ANALYZE_BASE", "ANALYZE_DELTA",
                                         "ANALYZE_ARCHIVE")
        ...
        *********************
        ...
        arcpy.RebuildIndexes_management(Sworkspace, "NO_SYSTEM", oDataList, "ALL")

        arcpy.AnalyzeDatasets_management(Sworkspace, "NO_SYSTEM", oDataList, "ANALYZE_BASE", "ANALYZE_DELTA",
                                         "ANALYZE_ARCHIVE")
        ...
        *************************
        ...
        replicaVersions = [replica.version for replica in arcpy.da.ListReplicas(Sworkspace)]
        ...
        if len(verReconcileList) > 0:  # check to see that the list is not empty
            for version in verReconcileList:
                try:
                    arcpy.DeleteVersion_management(Sworkspace, version)
                except:
